...
L0170 is an authoring language for fetching and transforming data, inspired by dplyr and jq. It fetches JSON or CSV from external or public web URLs (or accepts inline records), then composes a pipeline of operations — filter, select, mutate, group, sort, take, join, flatten, unique, format — to produce the result. Input is a natural-language description of where the data comes from and how to reshape it; output is an L0170 program that runs the pipeline and returns the transformed data. L0170 is the right tool when the job is "get this data from the web", "reshape this data", or "compute this summary"; it is not a chart renderer, a full programming language, or a persistent-storage interface.
